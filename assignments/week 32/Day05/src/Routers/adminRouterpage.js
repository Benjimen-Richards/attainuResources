...
adminrouter.route("/allusers").get((req, res) => {
  ...
  database
    ...
    .toArray((err, result) => {
      return res.render("admin", {
        data: result,
        userdata: req.session.logintoken,
      });
      // return res.send(result)
    });
});

adminrouter.route("/getuserdetails/:id").get((req, res) => {
  if (!req.session.logintoken) {
    return res.redirect("/?message=Login to continue");
  }
  if (req.session.logintoken.role !== "Admin") {
  ...
  database
    .collection(collection_name)
    .findOne({ _id: mongo.ObjectID(req.params.id) }, (err, result) => {
      return res.render("edituserpage", { data: result });
    });
});
adminrouter.route("/adminedituser/:id").post((req, res) => {
...
adminrouter.route("/deleteuser/:id").get((req, res) => {
  ...
  database
    .collection(collection_name)
    .removeOne({ _id: mongo.ObjectID(id) }, (err, result) => {
      if (err) throw err;
      res.redirect("/admin/allusers");
    });
});
...
adminrouter.route("/postuser").post((req, res) => {
  ...
  database
    .collection(collection_name)
    .findOne({ name: user.name, email: user.email }, (err, result) => {
      if (result) {
