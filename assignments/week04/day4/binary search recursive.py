#code
def binarysearchusingrecursive(s,l,r,t):
    if l>r:
        return
    mid=(l+r)//2
    if s[mid]==t:
        return True
    if s[mid]>t:
        return binarysearchusingrecursive(s,l,mid-1,t)
    elif s[mid]<t:
        return binarysearchusingrecursive(s,mid+1,r,t)
    ...
    for j in range(1):
        ...
        if len(string)>target[0]:
            ...
            if target[1] in string:
                left=0
                right=len(string)
                print(binarysearchusingrecursive(string,left,right,target[1]))
            else:
                print(False)
